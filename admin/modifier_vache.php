...
$stmt = $pdo->prepare('SELECT id, nom, bovin, date_naissance, age, poids, description, image, statut FROM vaches WHERE id = :id AND id_admin = :id_admin');
        ...
        <div class="panel-body">
          <div class="form-grid">
            <div class="form-group">
              ...
              <input type="text" id="nom" name="nom" list="racesList" placeholder="Choisir ou saisir une race" required autocomplete="off" value="<?php echo htmlspecialchars($vache['nom'] ?? ''); ?>">
            ...
            <div class="form-group">
              ...
              <input type="text" id="bovin" name="bovin" list="bovinsList" placeholder="Choisir ou saisir un type" required autocomplete="off" value="<?php echo htmlspecialchars($vache['bovin'] ?? 'Vache'); ?>">
            ...
            <div class="form-group">
              ...
              <input type="date" id="date_naissance" name="date_naissance" max="<?php echo date('Y-m-d'); ?>" value="<?php echo htmlspecialchars($vache['date_naissance'] ?? ''); ?>" required>
            </div>
            <div class="form-group">
...
<script>
// ===== Multi-image manager (edit mode) =====
const MAX_IMAGES = 5;

// Existing images from server
let existingImages = <?php echo json_encode(getVacheImages($vache['image'] ?? null)); ?>;
let deletedPaths = []; // paths to delete on submit
...
function renderGallery() {
  ...
  existingImages.forEach((path, index) => {
    ...
    const imgUrl = '../' + path.replace(/^\//, '');
    slot.innerHTML = '<img src="' + imgUrl + '" alt="Photo existante ' + (index + 1) + '">' +
      '<button type="button" class="img-remove" data-type="existing" data-index="' + index + '" title="Supprimer">' +
  ...
  newFiles.forEach((item, index) => {
    const slot = document.createElement('div');
    slot.className = 'img-slot filled';
    slot.innerHTML = '<img src="' + item.previewUrl + '" alt="Nouvelle photo ' + (index + 1) + '">' +
      '<button type="button" class="img-remove" data-type="new" data-index="' + index + '" title="Supprimer">' +
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg></button>';
    gallery.insertBefore(slot, addSlot);
  });

  addSlot.style.display = totalImages() >= MAX_IMAGES ? 'none' : 'flex';
...
gallery.addEventListener('click', (e) => {
  ...
  if (type === 'existing' && index >= 0 && index < existingImages.length) {
    deletedPaths.push(existingImages[index]);
    existingImages.splice(index, 1);
    renderGallery();
  } else if (type === 'new' && index >= 0 && index < newFiles.length) {
